...
public class Demo02_connection {
    public static void main(String[] args) throws Exception {
        ...
        /*
        #Connection 接口（用于通讯，发送sql，返回结果）
            1）创建sql语句 Statement statement = connection.createStatement();（了解，以后不会用）
            2）预编译：PreparedStatement pstm = statement.preparedstatement;
            3)事务操作
                start transaction；
                commit；
                rollback；

                connection.setAutonCommit(false);禁止mysql自动提交事务，即开启事务
                connection.commit();
                connetction.rollback();
